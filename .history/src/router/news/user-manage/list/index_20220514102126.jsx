    ...
    render:(items)=>{
      ...
      return <div>
         <Button danger  onClick={()=> delItem(items)} shape='circle' icon = {<DeleteOutlined/>} style = {{marginRight:'10px'}} disabled = {items.default}/>
          ...
          <Button type = 'primary' shape='circle' icon = {<EditOutlined/>}  disabled = {items.default} onClick = {(items) => setisUpdate(true)}/>
          <Modal
            visible={isUpdate}
            title="更新用户"
            okText="确定更新"
            ...
            onOk={handleedit}
          >
            <UserForm roleList = {roleList} regList = {regList} ref = {upform}/>
          </Modal>
      </div>
    }
  },
...
const  permisson = (items) =>{
 ...
 const  handleedit = (items) =>{
   ...
   upform.current.validateFields(items)
