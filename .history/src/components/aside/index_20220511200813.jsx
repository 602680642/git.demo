...
/*const menuList = [
 ...
 const iconList = {
  ...
  "/news-manage/right":<UserOutlined/>,
  ...
  const renderMenu = (menu) =>{
      ...
      return  menu.map(item =>{
        ...
        if(item.children?.length>0 && pagepermisson(item)){
          ...
          return <SubMenu key={item.key} title={item.title} icon = {iconList[item.icon]}>

                  {renderMenu(item.children)} 
              </SubMenu>
        }

        return pagepermisson(item) && <Menu.Item key={item.key} onClick = {() => props.history.push(item.key)} icon={iconList[item.icon]} >{item.title}</Menu.Item>
      })
  }
